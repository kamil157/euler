...
The advantage with the XOR function is that using the same
encryption key on the cipher text, restores the plain text;
...
def find_words(text, english_words):
    return (word for word in ''.join(text).split()
...
def solve():
    ...
    count, key, text = max(decipher(cipher, english_words))
    return sum(ord(c) for c in text)


def decipher(cipher, english_words):
    for key in product([ord(c) for c in ascii_lowercase], repeat=3):
        text = decode(cipher, key)
        count = len(list(find_words(text, english_words)))
        yield count, key, text
